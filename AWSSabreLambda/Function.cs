...
namespace AWSSabreLambda
{
    public class Function
    {
        ...
        private const string API_KEY = @"[INSERT YOUR API KEY HERE]";

        //sample usage: alexa ask schedule info from new york city to los angeles
        //while in session: from boston to mexico city
            ...
            if (requestType == typeof(IntentRequest))
            {
                if (input.Request is IntentRequest intentRequest)
                {
                    if (intentRequest.Intent.Slots.ContainsKey("Origin") && intentRequest.Intent.Slots.ContainsKey("Destiny"))
                    {
                        ...
                        var response = await GetItineraryInfo(originCode, destinyCode, context);
        ...
        private async Task<string> GetItineraryInfo(string originCode, string destinyCode, ILambdaContext context, DateTime? date = null)
        {
